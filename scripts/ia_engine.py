...
def guardar_log_ia(respuesta, equipo_local):
    try:
        ...
        fila = pd.DataFrame([{
            ...
            "diferencial": respuesta.get("prediccion", {}).get("diferencial_estimado", ""),
            "ganador": respuesta.get("prediccion", {}).get("ganador_probable", ""),
            "probabilidad": respuesta.get("prediccion", {}).get("probabilidad_ganador_local", ""),
            "tendencia": respuesta.get("tendencia_prophet", ""),
            "consistencia": respuesta.get("consistencia_automl", ""),
        ...
        print(f"⚠️ Error al guardar log IA: {str(e)}")
...
def procesar_evento(evento_json):
    try:
        ...
        trend = forecast_trend_prophet(evento_json.get("equipo_local", ""))
        ...
        guardar_log_ia(respuesta, evento_json.get("equipo_local", "Desconocido"))
